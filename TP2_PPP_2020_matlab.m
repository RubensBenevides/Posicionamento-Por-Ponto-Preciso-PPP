...
clear
%********************************************************
% INICIALIZAR CONSTANTES E PARÂMETROS
%********************************************************
...
L2=[84653910.02547;83790961.72746;
    ...
    97107947.98744];
% Combinação ion-free para corrigir o erro ionosférico na propagação das ondas:
PD_L0   = (m1*C1)-(m2*P2);
Fase_L0 = (m1*L1)-(m2f*L2);
Lb      = [PD_L0;Fase_L0];
RMSEx = zeros(100,1);
% Pesos das observáveis que será utilizado no ajustamento (minimização das funções):
...
% ATRASO NOS HARDWARES (TGD) (tabelado no arquivo  RINEX ppte1971.12n)
...
% Modelo de Saastamoinen para correção do erro de propagação na Troposfera:
...
% Calcular distância média terra-lua com o semi-eixo e a excentricidade da órbita:
...
% Correção para o efeito de maré da LUA:
for n=1:3
...
% ERRO RELATIVÍSTICO
%*******************************************************
% 1 - Carregar parâmetros orbitais de cada PRN (cada satélite)
...
% 2 - Propagar os parâmetros para tt_GPS (tempo de transmissão dos satélites):
for n=1:SATs 
    nk(n,1)=(((GM_T/(sqr_a(n))^6))^(1/2))+vn(n); % mov. ang. no toe
    tk(n,1)=tt_GPS(n)-toe(n);                
    Mk(n,1)=M0(n)+(nk(n)*tk(n));              
    % Calcular anomalia média de cada PRN pelo método iterativo de Newton:
...
% CORREÇÃO DE RELATIVIDADE E ATRASO DE HARDWARE NOS dts (dtsC = dts Corrigido):
...
while (~fim)
    ...
    L0 = [PD;FASE];
    L  = L0 - Lb;
    % Componentes da matriz A (Matriz Jacobiana, derivadas das equações em relação às observações):
    ...
    N   = A'*Peso*A;
    U   = A'*Peso*L;
    Xc  = -inv(N)*U; % A correção dos parâmetros está aqui em cada iteração
    Xa  = Xo+Xc;
    Xo  = Xa;
    dtr = Xo(4)/c;
    erro_max = max(abs(Xc));
...
itera
gl=14-11; % graus de liberdadde (nº de eq. menos o nº de variáveis) 
V=A*Xc+L  % resíduos das observações (ou seja, as pseudodistâncias e fases)
...
% Comparar valor calculado com o verdadeiro:
Erro_x = Xa(1)-(3687624.3674)
Erro_y = Xa(2)-(-4620818.6827)
Erro_z = Xa(3)-(-2386880.3805)
# Métrica dada pela norma euclidiana:
Erro_xyz = norm([Erro_x,Erro_y,Erro_z])
% Média quadrática do erro total com a variância à posteriori:
EQM = sqrt((Erro_xyz^2+var_posteriori^2)/2)
