...
C1=[20673352.930;20462608.391;
    23710479.281;21504467.148;    
    21432292.453;22830292.500;
    23714758.398];
...
%*******************************************************
%P_PD   = diag(sind(elev).^2).*inv(3^2);
%P_fase = diag(sind(elev).^2).*inv(0.1^2); 
%Peso   = [P_PD    zeros(7);
%          zeros(7) P_fase];
...
range = [67,6,74,11]; RA_DEC = dlmread('horizons_luna.txt'," ",range);
...
range = [66,6,73,11]; RA_DEC = dlmread('horizons_sun.txt'," ",range);
RA  = RA_DEC(:,1) + RA_DEC(:,2)/60 + RA_DEC(:,3)/3600; 
DEC = RA_DEC(:,4) + RA_DEC(:,5)/60 + RA_DEC(:,6)/3600;
...
% Calcular distância terra-lua
range = [15,4,15,25]; aelua = dlmread('horizons_luna.txt',"",range);
a = aelua(1); b = aelua(1)*sqrt(1-aelua(5)^2); Rlua = sqrt(a*b)*1000;
...
qui_quadrado_tabelado_95 = 12.84;
if qui_quadrado < qui_quadrado_tabelado_95
   printf('Passou no teste qui-quadrado (valor calculado = %.2f < 7.82 valor tabelado)\n', qui_quadrado);
else
   printf('Falhou no teste qui-quadrado (valor calculado = %.2f > 7.82 valor tabelado)\n', qui_quadrado);
end
% Métricas de Erro
Erro_em_x = Xa(1)-(3687624.3674)
Erro_em_y = Xa(2)-(-4620818.6827)
Erro_em_z = Xa(3)-(-2386880.3805)
Erro_total = norm([Erro_em_x,Erro_em_y,Erro_em_z])
RMSE = norm([Erro_total,var_posteriori])
